...
function App(): React.JSX.Element {
  ...
  const handleImport = useCallback((importedData: SubjectData[]) => {
    importedData.forEach(importedItem => {
      ...
      if (!existsLocally) {
        addSubject(importedItem.title, importedItem.color || DEFAULT_SUBJECT_COLOR)
      }
    })
  }, [subjects, addSubject])
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            title="View Stats"
          >
            <BarChart2 className="h-4 w-4 text-muted-foreground hover:text-foreground" />
          </Button>
          <Button
            variant="ghost"
            size="icon"
      ...
      <footer className="px-4 py-3 border-t shrink-0">
        <form onSubmit={handleAddSubject} className="flex gap-2">
          ...
          <Button
            type="submit"
            size="sm"
            className="h-8 w-7 p-0 shrink-0"
